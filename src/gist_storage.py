...
# ---------------------------------------------------------------------------
# Andrea Ravalli Style & Authentic Reply Archive (for AI Comment Responder)
# ---------------------------------------------------------------------------

def get_andrea_style_archive() -> list:
    """
    Return list of authentic replies and comments written by Andrea Ravalli stored in Gist.
    Used as few-shot exemplars to accurately capture his tone, depth, and vocabulary.
    """
    data = load_data()
    return data.get('andrea_style_archive', [])


def save_andrea_style_archive(archive: list) -> bool:
    """Save updated Andrea style archive to Gist."""
    data = load_data()
    data['andrea_style_archive'] = archive
    _invalidate_cache()
    return save_data(data)


def upsert_andrea_style_replies(new_replies: list) -> int:
    """
    Merge a list of reply dictionaries into Gist style archive, deduplicating by ID or content hash.
    Keeps up to 500 most recent authentic replies.
    Returns the count of newly added replies.
    """
    if not new_replies:
        return 0

    data = load_data()
    existing = data.get('andrea_style_archive', [])
    existing_ids = {r.get('id') for r in existing if r.get('id')}
    existing_texts = {r.get('andrea_reply', '').strip() for r in existing if r.get('andrea_reply')}

    added_count = 0
    for nr in new_replies:
        nr_id = nr.get('id')
        nr_text = nr.get('andrea_reply', '').strip()
        if not nr_text or len(nr_text) < 15:
            continue

        if (nr_id and nr_id in existing_ids) or (nr_text in existing_texts):
            continue

        existing.append(nr)
        if nr_id:
            existing_ids.add(nr_id)
        existing_texts.add(nr_text)
        added_count += 1

    if added_count > 0:
        if len(existing) > 500:
            existing = existing[-500:]
        data['andrea_style_archive'] = existing
        _invalidate_cache()
        save_data(data)
        print(f"✅ Upserted {added_count} new authentic replies into Andrea Style Archive (total: {len(existing)}).")
